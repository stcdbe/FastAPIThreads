...
from src.modules.user.views.schemas import UserGet, UserCreate, UserPatch
...
async def get_some_users(current_user: CurrentUserDep,
                         ...
                         params: Annotated[dict[str, Any], Depends(get_user_list_params)]) -> list[User]:
...
@user_router.get(path='/{guid}',
                 status_code=200,
                 response_model=UserGet,
                 name='Get the user')
async def get_user(current_user: CurrentUserDep,
                   user: Annotated[User, Depends(validate_user_guid)]) -> User:
    return user
